...
const callClaude = async (systemPrompt: string, userPrompt: string): Promise<string> => {
  if (structuralMode) {
    ...
    if (sp.includes('critique') || sp.includes('design critique')) {
      return JSON.stringify({ overallScore: 78, categories: { visualDesign: 82, usability: 75, accessibility: 70, performance: 80, content: 78 }, strengths: ['Clean layout', 'Good CTA placement'], weaknesses: ['Contrast issues in footer', 'Missing alt text on some images'], recommendations: ['Improve color contrast ratio', 'Add descriptive alt text'] });
    }
    ...
    if (sp.includes('component') && (sp.includes('reconstruct') || sp.includes('converting'))) {
      return JSON.stringify([{ name: 'HeroSection', originalType: 'hero', tsx: '<div className="hero">Hero</div>', tailwindClasses: ['bg-gradient-to-r', 'from-orange-500'], propsInterface: 'interface HeroProps { title: string; subtitle: string; ctaText: string; }', ariaAttributes: ['role="banner"'], stateVariants: ['hover', 'focus'], responsive: true, storybookStory: 'export default { title: "HeroSection" }; export const Default = () => <HeroSection />;', variants: { default: true, dark: true }, props: ['title', 'subtitle', 'ctaText'] }, { name: 'WorkoutCard', originalType: 'card', tsx: '<div className="workout-card">Card</div>', tailwindClasses: ['rounded-xl', 'shadow-md'], propsInterface: 'interface WorkoutCardProps { name: string; duration: number; difficulty: string; }', ariaAttributes: ['role="article"'], stateVariants: ['hover', 'loading'], responsive: true, storybookStory: 'export default { title: "WorkoutCard" }; export const Default = () => <WorkoutCard />;', variants: { default: true, loading: true }, props: ['name', 'duration', 'difficulty'] }, { name: 'ProgressRing', originalType: 'visualization', tsx: '<svg className="progress-ring">Ring</svg>', tailwindClasses: ['animate-spin'], propsInterface: 'interface ProgressRingProps { progress: number; size: number; color: string; }', ariaAttributes: ['role="progressbar"', 'aria-valuenow="0"'], stateVariants: ['default', 'complete'], responsive: true, storybookStory: 'export default { title: "ProgressRing" }; export const Default = () => <ProgressRing />;', variants: { default: true, complete: true }, props: ['progress', 'size', 'color'] }]);
    }
    ...
    if (sp.includes('design review') || sp.includes('review specialist')) {
      return JSON.stringify({ overallScore: 82, findings: [{ phase: 'Visual Polish', severity: 'medium', title: 'Contrast ratio on secondary text', description: 'Some body text has insufficient contrast', impact: 'Readability for low-vision users', affectedComponents: ['ProgressRing'] }], phaseScores: { interaction: 85, responsiveness: 80, visualPolish: 82, accessibility: 78, robustness: 80, codeHealth: 85, content: 83 }, summary: 'Strong design with minor accessibility improvements needed', strengths: ['Consistent visual language', 'Good color palette', 'Clear hierarchy'], criticalIssues: [], recommendations: ['Increase contrast on secondary text', 'Add focus indicators to interactive elements'], accessibilityScore: 78, responsiveScore: 80, visualPolishScore: 82, interactionScore: 85, reviewedAt: Date.now() });
    }
